import { EMBED_TO, EMBED_URL, EMBED_VIDSRC, IMAGE_URL } from "./constants";
...
export const getLinkIframe = (type: 'movie' | 'tv', id: number, season?: number, episode?: number ) => {
  const searchParams = new URLSearchParams(window.location.search);
  
  let srv = searchParams.get('srv')
  if(!srv) {
    srv = "2EMBED"
  }
 
  if(srv === "2EMBED") {
    return type === "movie" ? `${EMBED_URL}/${id}` :  `${EMBED_VIDSRC}/${id}/${season}-${episode}`;
  }

  if(srv === "VIDSRC") {
    return type === "movie" ? `${EMBED_VIDSRC}/${id}`:  `${EMBED_VIDSRC}/${id}/${season}-${episode}`;
  }
